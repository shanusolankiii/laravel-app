    ...
    public function saveClinic(Request $request)
    {
        ...
        if ($request->all()) {
            ...
            $submission = PractitionerSubmission::where(
                [
                    ['session_id', '=', $session_id],
                    ['question_id', '=', 'q_vc_postal'],
                ]
            )->get();
           if ($submission->isEmpty()) {
                $data =[
                     ['session_id' => $session_id, 'question_id' => 'q_vcplace_id', 'question_title' => $this->default_questions['q_vcplace_id'],'answer' => trim($clinic_info['q_vcplace_id']),'created_at' => date("Y-m-d h:i:s"),
                'updated_at' => date("Y-m-d h:i:s")],
                    ['session_id' => $session_id, 'question_id' => 'q_vclatitude', 'question_title' => $this->default_questions['q_vclatitude'], 'answer' => trim($clinic_info['q_vclatitude']),'created_at' => date("Y-m-d h:i:s"),
                    'updated_at' => date("Y-m-d h:i:s")],
                    ['session_id' => $session_id, 'question_id' => 'q_vclongitude', 'question_title' => $this->default_questions['q_vclongitude'], 'answer' => trim($clinic_info['q_vclongitude']),'created_at' => date("Y-m-d h:i:s"),
                    'updated_at' => date("Y-m-d h:i:s")],
                    ['session_id' => $session_id, 'question_id' => 'q_vc_postal', 'question_title' => $this->default_questions['q_vc_postal'], 'answer' => trim($clinic_info['q_vc_postal']),'created_at' => date("Y-m-d h:i:s"),
                    'updated_at' => date("Y-m-d h:i:s")]
                ];
                ...
                if ($added_virtual) {
                    ...
                    if (!$practitioner_master->isEmpty()) {
                        ...
                        if (is_null($course_id)) {
                            ...
                            $response = Artisan::call('hb:approve:naturopath --session_id='.$session_id);                        
                            return response()->json([
                                "status" => 200,
                                "message" => "Approved the Practitioners!",
                                "data" => [],
                            ], 200);
                        } else {
                            $this->deleteCookie();
            ...
            $session_id = $this->getCookie();
            $model = Practitioner::where('session_id', $session_id);
            $q_vc_postal = PractitionerSubmission::where(
                [
                    ['session_id', '=', $session_id],
                    ['question_id', '=', '  '],
                ]
